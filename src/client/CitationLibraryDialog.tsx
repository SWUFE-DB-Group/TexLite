...
export function CitationLibraryDialog({ open, onOpenChange, page = false, onBack, currentFile = "", currentSource = "", readOnly = false, currentUserId, onInsert }: Props) {
  ...
  const libraryBody = <div className="citation-library-dialog">
      ...
      {view === "library" ? <>
        ...
        {loading ? <div className="citation-library-empty"><LoaderCircle className="spin" size={22} /><span>{t("common.loading")}</span></div> : entries.length === 0 ? <div className="citation-library-empty"><BookMarked size={28} /><strong>{t("citationLibrary.emptyTitle")}</strong><span>{selectedTag ? t("citationLibrary.noTagMatches") : t("citationLibrary.emptyDescription")}</span></div> : <div className="citation-entry-list">{entries.map((entry) => <CitationCard key={entry.id} entry={entry} currentUserId={currentUserId} importDisabled={readOnly} deleting={deletingId === entry.id} onImport={onInsert ? async () => { if (await onInsert(entry)) onOpenChange(false); } : undefined} onEdit={entry.ownerId === currentUserId ? () => { setTagEditor(null); setAdding(false); setEditing(entry); setEditText(entry.bibtex); } : undefined} onTags={entry.ownerId === currentUserId ? () => editTags(entry) : undefined} onDelete={entry.ownerId === currentUserId ? () => void deleteEntry(entry) : undefined} t={t} />)}</div>}
        ...
        {currentEntries.length === 0 ? <div className="citation-library-empty"><BookMarked size={28} /><strong>{t("citationLibrary.noEntries")}</strong><span>{t("citationLibrary.noEntriesDescription")}</span></div> : <div className="citation-entry-list">{currentEntries.map((entry, index) => <CitationCard key={`${entry.citationKey}-${index}`} entry={entry} saved={savedEntries.has(entry.citationKey.toLowerCase())} saving={savingKey === entry.citationKey} saveDisabled={lookupLoading} onSave={() => void saveEntry(entry)} t={t} />)}</div>}
      </>}
      {tagEditor && <div className="citation-tag-editor"><div className="citation-edit-header"><strong>{t("citationLibrary.tagsTitle", { key: tagEditor.citationKey })}</strong><button type="button" aria-label={t("common.close")} onClick={() => setTagEditor(null)}><X size={15} /></button></div><div className="citation-tag-choice-list">{tags.length === 0 ? <span className="muted">{t("citationLibrary.noTagsToAssign")}</span> : tags.map((tag) => <label key={tag.id}><input type="checkbox" checked={tagEditorIds.includes(tag.id)} onChange={() => setTagEditorIds((current) => current.includes(tag.id) ? current.filter((id) => id !== tag.id) : [...current, tag.id])} /><span className={`citation-tag-dot tag-${tag.color}`} />{tag.name}</label>)}</div><div className="citation-edit-actions"><button type="button" onClick={() => setTagEditor(null)}>{t("common.cancel")}</button><button type="button" className="primary" disabled={tagSaving || tags.length === 0} onClick={() => void saveTags()}>{tagSaving ? <LoaderCircle className="spin" size={14} /> : <Check size={14} />}{t("citationLibrary.saveTags")}</button></div></div>}
      {editing && <div className="citation-edit-panel"><div className="citation-edit-header"><strong>{t("citationLibrary.editTitle", { key: editing.citationKey })}</strong><button type="button" aria-label={t("common.close")} onClick={() => setEditing(null)}><X size={15} /></button></div><textarea value={editText} onChange={(event) => setEditText(event.target.value)} spellCheck={false} rows={12} /><div className="citation-edit-actions"><button type="button" onClick={() => setEditing(null)}>{t("common.cancel")}</button><button type="button" className="primary" disabled={!editText.trim() || savingKey === editing.citationKey} onClick={() => void saveEdited()}>{savingKey === editing.citationKey ? <LoaderCircle className="spin" size={14} /> : <Check size={14} />}{t("common.save")}</button></div></div>}
    </div>;
  if (page) {
...
function CitationCard({ entry, currentUserId, saved, saving, saveDisabled, importDisabled, deleting, onSave, onImport, onEdit, onTags, onDelete, t }: {
  ...
  return <article className="citation-entry-card"><div className="citation-entry-main"><div className="citation-entry-heading"><code>{entry.citationKey}</code><span>@{entry.entryType}</span>{saved && <em><Check size={11} />{t("citationLibrary.savedLabel")}</em>}</div><strong>{entry.title || t("citationLibrary.untitled")}</strong><p>{entry.authors || t("citationLibrary.authorUnknown")}{entry.year && <span> · {entry.year}</span>}</p>{libraryEntry && libraryEntry.tags.length > 0 && <div className="citation-entry-tags">{libraryEntry.tags.map((tag) => <span className={`citation-tag-pill tag-${tag.color}`} key={tag.id}><span className="citation-tag-dot" />{tag.name}</span>)}</div>}</div><div className="citation-entry-actions">{onSave && <button type="button" className={saved ? "saved" : "primary"} disabled={saving || saveDisabled} onClick={onSave}>{saving || saveDisabled ? <LoaderCircle className="spin" size={13} /> : saved ? <Check size={13} /> : <Plus size={13} />}{saving ? t("citationLibrary.saving") : saved ? t("citationLibrary.update") : t("citationLibrary.save")}</button>}{onImport && <button type="button" disabled={importDisabled} title={importDisabled ? t("citationLibrary.importRequiresWrite") : t("citationLibrary.importToFile")} onClick={() => void onImport()}><Import size={13} />{t("citationLibrary.import")}</button>}{libraryEntry && onTags && <button type="button" onClick={onTags}><Tags size={13} />{t("citationLibrary.tags")}</button>}{isLibraryEntry && onEdit && <button type="button" onClick={onEdit}><Edit3 size={13} />{t("common.edit")}</button>}{isLibraryEntry && onDelete && <button type="button" className="danger-text" disabled={deleting} onClick={onDelete}>{deleting ? <LoaderCircle className="spin" size={13} /> : <Trash2 size={13} />}{t("common.delete")}</button>}</div></article>;
}
