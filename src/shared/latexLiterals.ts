// Literal TeX syntax, not all environments that lack prose. In particular,
// algorithmic and alltt still interpret LaTeX commands.
export const literalEnvironmentNames = [
  ...
  "verbatimwrite", "lstlisting", "lstlisting*", "minted", "minted*", "tcblisting", "tcblisting*", "pygmented",
...
export function inlineLatexLiteralEnd(source: string, start: number, end = source.length): number | null {
  ...
  if (delimiter === "{" && (name === "lstinline" || name === "mintinline")) {
    return skipRequiredArgument(source, position, end) ?? end;
  }
